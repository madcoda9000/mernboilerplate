...
router.post('/startMfaSetup', auth, async (req, res) => {
	...
	try {
		...
		if (!user) {
			...
			return res
				.status(400)
				.json({ error: true, message: "No user with id " + req.body._id + " exist!" })
		}
...
router.post("/finishMfaSetup", auth, async (req, res) => {
	...
	try {
		...
		if (req.body.otpToken === null) {
			...
			return res
				.status(400)
				.json({ error: true, message: "The User otpToken is required!" })
		}
		...
		let erg = await User.findByIdAndUpdate({ _id: user._id }, update);
...
router.post(("/validateOtp"), auth, async(req,res) => {
	...
	try {
		...
		if (delta === null) {
			doHttpLog('RES', mid, req.method, req.originalUrl, req.ip, "OTP Token is invalid!", 400);
		...
		let erg = await User.findByIdAndUpdate({ _id: user_id }, update);
...
router.post("/confirmEmail", async (req, res) => {
	...
	try {
		...
		if (mailuser.emailVerifyToken !== req.body.token) {
			...
			return res
				.status(400)
				.json({ error: true, message: "Confirmation token is invalid!" });
		}
		...
		res
			.status(200)
			.json({ error: false, message: "Account " + req.body.email + " confirmed email sucessfully" });
	} catch (err) {
		console.log(err);
		doHttpLog('RES', mid, req.method, req.originalUrl, err.message, 500);
...
router.post("/logIn", async (req, res) => {
	...
	try {
		...
		if (user.accountLocked === true) {
			...
			return res
				.status(401)
				.json({ error: true, message: "Your account is locked! Please contact your administrator." });
		}
		...
		let erg = User.findByIdAndUpdate({_id:user._id}, update);

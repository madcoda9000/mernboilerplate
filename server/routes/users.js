...
router.post("/forgotPw1", async (req, res) => {
	...
	try {
		...
		if(req.body.email===undefined || req.body.email===null) {
			...
			return res
					.status(400)
					.json({ error: true, message: "The email parameter is required!" })		
		}	
		...
		if(user && user.pwResetToken.trim()!=="") {
			...
			return res
				.status(200)
				.json({ error: true, message: "Your account is locked currently due to an open password reset request! Please finish your password reset or contact your administrator." });
		}
		...
		if(user) {
			...
			var erg = await User.findByIdAndUpdate({_id:user._id}, update);
			await sendPwResetMail(user, token);
			doHttpLog('RES',mid,req.method,req.originalUrl,req.ip,"Account deactivated, password reset mail sended successfully. Please take a look into your inbox.",200);
			res
			.status(200)
			.json({ error: false, message: "Account deactivated, password reset mail sended successfully. Please take a look into your inbox." });
...
router.get("/user/:userId", auth, async (req, res) => {
	...
	try {
		...
		if(req.params.userId===undefined || req.params.userId===null) {
			...
			return res
					.status(400)
					.json({ error: true, message: "The userId parameter is required!" })		
		}
...
router.post("/createUser", auth, roleCheck("admins"), async (req, res) => {
	...
	try {		
		...
		res
			.status(201)
			.json({ error: false, message: "Account " + req.body.email + " created sucessfully" });
	} catch (err) {
		doHttpLog('RES',mid,req.method,req.originalUrl,err.message,500);
...
router.post("/deleteUser", auth, roleCheck("admins"), async (req, res) => {
	try {
		...
		if(!iduser) {
			...
			res
				.status(200)
				.json({ error: false, message: "User deleted sucessfully" });
		}
	} catch (err) {
		doHttpLog('RES',mid,req.method,req.originalUrl,err.message,500);
...
router.put("/updateUser", auth, roleCheck("admins"), async (req, res) => {
	...
	try {		
		...
		res
			.status(201)
			.json({ error: false, message: "Account " + req.body.userName + " updated sucessfully" });
	} catch (err) {
		doHttpLog('RES',mid,req.method,req.originalUrl,err.message,500);
...
router.patch("/changeEmailAddress", auth, async (req, res) => {
	...
	res
		.status(200)
		.json({ error: false, message: "Account " + iduser.userName + " updated sucessfully" });
});
...
router.patch("/changePassword", auth, async (req, res) => {
	...
	if(!verifiedPassword) {
		...
		res
			.status(201)
			.json({ error: false, message: "Account " + iduser.userName + " changed password sucessfully" });
	}	
});
...
router.get("/paginated/:page/:pageSize/:searchParam?", auth, roleCheck("admins"), async (req, res) => {
	...
	try {		
		...
		if(req.params.searchParam) {
			var query = {$or: [{userName: { $regex: '.*' + req.params.searchParam + '.*' }},{firstName: { $regex: '.*' + req.params.searchParam + '.*' }}, {lastName: { $regex: '.*' + req.params.searchParam + '.*' }}, {email: { $regex: '.*' + req.params.searchParam + '.*' }}] };
...
router.patch("/lockUser", auth, roleCheck("admins"), async (req, res) => {
	try {
		...
		res
			.status(200)
			.json({ error: false, message: "Account " + req.body._id + " locked sucessfully" });
	} catch (err) {
		doHttpLog('RES',mid,req.method,req.originalUrl,err.message,500);
...
router.patch("/unlockUser", auth, roleCheck("admins"), async (req, res) => {
	try {
		...
		res
			.status(200)
			.json({ error: false, message: "Account " + req.body._id + " unlocked sucessfully" });
	} catch (err) {
		doHttpLog('RES',mid,req.method,req.originalUrl,err.message,500);
...
router.patch("/enableLdap", auth, roleCheck("admins"), async (req, res) => {
	try {
		...
		res
			.status(200)
			.json({ error: false, message: "Account " + req.body._id + " ldap enabled sucessfully" });
	} catch (err) {
		doHttpLog('RES',mid,req.method,req.originalUrl,err.message,500);
...
router.patch("/disableLdap", auth, roleCheck("admins"), async (req, res) => {
	try {
		...
		res
			.status(200)
			.json({ error: false, message: "Account " + req.body._id + " ldap disabled sucessfully" });
	} catch (err) {
		doHttpLog('RES',mid,req.method,req.originalUrl,err.message,500);
...
router.patch("/enforceMfa", auth, roleCheck("admins"), async (req, res) => {
	try {
		...
		res
			.status(200)
			.json({ error: false, message: "Account " + req.body._id + " mfa enforced sucessfully" });
	} catch (err) {
		doHttpLog('RES',mid,req.method,req.originalUrl,err.message,500);
...
router.patch("/unenforceMfa", auth, roleCheck("admins"), async (req, res) => {
	try {
		...
		res
			.status(200)
			.json({ error: false, message: "Account " + req.body._id + " mfa enforcement disabled sucessfully" });
	} catch (err) {
		doHttpLog('RES',mid,req.method,req.originalUrl,err.message,500);
 ...
 * }
 */
router.patch("/disableMfa", auth, roleCheck("admins"), async (req, res) => {
	try {
		const mid = crypto.randomBytes(16).toString("hex");
		doHttpLog('REQ',mid,req.method,req.originalUrl,req.ip);
		...
		await User.findByIdAndUpdate({_id:req.body._id}, update);
		doHttpLog('RES',mid,req.method,req.originalUrl,req.ip,"Account " + req.body._id + " mfa disabled sucessfully",200);
		res
			.status(200)
			.json({ error: false, message: "Account " + req.body._id + " mfa disabled sucessfully" });
	} catch (err) {
		doHttpLog('RES',mid,req.method,req.originalUrl,err.message,500);

...
router.get("/getQuoteOfTheDay", async (req, res) => {
  ...
  const count = await Quote.countDocuments({ genre: "computers" })
  ...
  try {
    if (count !== null) {
      ...
      const result = await Quote.findOne({ genre: "computers" }).skip(random)
      if (result !== null) {
        // Tada! random quote
        doHttpLog(
          "RES",
          ...
          req.originalUrl,
          req.ip,
          "Quote of the day fetched sucessfully",
          200
        )
        res
          .status(200)
          .json({ error: false, message: "Quote of the day fetched sucessfully", quote: result })
      }
    }
  } catch (err) {

...
const SystemLogs = () => {
  ...
  useEffect(() => {
    SetIsLoading(true);
    makeAuditEntry(user.userName, "info", "viewed System logs");
    var pSize = 100;
    var searchStr;
    ...
    if (search) {
      makeAuditEntry(user.userName, "info", "searched system logs for " + search);
    }
    LogsService.getSystemLogs(1, pSize, searchStr).then((response) => {
      if (!response.error) {
  ...
  const LoadData = (page, pageSize) => {
    SetIsLoading(true);
    LogsService.getSystemLogs(page, pageSize, "").then((response) => {
      if (!response.error) {
        setLogData(response.data.paginatedResult);
        SetIsLoading(false);
        setSelectedPage(response.data.paginatedResult.page);
                  ...
                  <Tbody>
                    {LogData.docs.map((log, i) => (
                      <Tr key={i}>
                        ...
                        <Td>
                          {log.level === "info" && <MdInfo color="var(--chakra-colors-blue-500)" size={20} title="Informational log entry" />}
                          {log.level === "warning" && <MdOutlineWarning color="var(--chakra-colors-yellow-500)" size={20} title="Warning log entry" />}
                          {log.level === "error" && <MdError color="var(--chakra-colors-red-500)" size={20} title="Error log entry" />}
                        </Td>

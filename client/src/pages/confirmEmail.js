...
const ConfirmEmail = () => {
    ...
    useEffect(() => {
        ...
        } else { setFormData((prevData) => ({ ...prevData, _id: searchParams.get('_id').trim() })); }
        if (searchParams.get('token')===null || !searchParams.get('token').trim()) {
            addError("Token is missing!");
        } else { setFormData((prevData) => ({ ...prevData, token: searchParams.get('token').trim() })); }
        if (searchParams.get('email')===null || !searchParams.get('email').trim()) {
            addError("User email address is missing!");
        } else { setFormData((prevData) => ({ ...prevData, email: searchParams.get('email').trim() })); }
            ...
            <>
                ...
                {errMsg &&
                    <div className='chakra-alert'
                        ...
                        <Heading fontSize={'lg'} style={{ marginBottom: '10px' }}>The following errors occured!</Heading>

      ...
      <VStack> 
        <Box mt={2} ml={0} w={'100%'}>   
          {LinkItems.map((link, i) => (            
            ...
            {link.childrens ? 
              (
                <Box  ml={0} mb={2} w={'100%'} onClick={link.trigger.toggle} transition="3s ease" display={link.admin===true && !user.roles.includes("admins") ? 'none' : 'display'}>
                  <NavLink link={link} />
                  {link.childrens && (
                    <Box w={'90%'} ml={6}  transition="3s ease" >
                    {link.childrens.map((child, ci) => (
                        <SubNavLink fontWeight={window.location.pathname.indexOf(child.label) > -1 ? 'bold' : 'normal'} key={ci} link={child} hidden={child.hider} display={child.admin===true && !user.roles.includes("admins") ? 'none' : 'display'}/>
                    ))}
                    </Box>
                  )}
                </Box>

...
const DarkModeSwitcher = ({ showSwitch, position }) => {
  ...
  useEffect(() => {
    ...
    if (cMode) {
      if (cMode === "light" && colorMode === "dark") {
        toggleColorMode();
      }
      if (cMode === "dark" && colorMode === "light") {
        toggleColorMode();
      }
    }
  ...
  if (showSwitch === true) {
    return (
      <Switch position={position} bottom={"10"} colorScheme="gray" id="dmSwDarkMode" onChange={setDarkMode} isChecked={colorMode === "light" ? false : true}>
        {colorMode === "light" ? <Icon color="gray.500" as={MdBrightness5} /> : <Icon as={MdBrightness2} />}
      </Switch>
    );
